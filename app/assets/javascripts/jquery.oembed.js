...
(function($) {
    ...
    function embedCode(container, externalUrl, embedProvider) {
      if ($('#jqoembeddata').data(externalUrl)!=undefined && embedProvider.embedtag.tag!='iframe'){
        ...
        if(embedProvider.embedtag.tag!==''){
          ...
          if(settings.tagClass) code.addClass(settings.tagClass);
          if(tag=='embed')
            code
    ...
    $.fn.oembed.getPhotoCode = function(url, oembedData) {
        ...
        if (oembedData.url){
            code = '<div><a href="' + url + '" target=\'_blank\'><img src="' + oembedData.url + '" alt="' + alt + '" class="'+settings.tagClass+'"/></a></div>';
        } else if (oembedData.thumbnail_url){
            var newURL = oembedData.thumbnail_url.replace('_s','_b');
            code = '<div><a href="' + url + '" target=\'_blank\'><img src="' + newURL + '" alt="' + alt  + '" class="'+settings.tagClass+ '"/></a></div>';
        } else {
...
//        if (oembedData.html) code += "<div>" + oembedData.html + "</div>";
            ...
            extraSettings.yql = {from:'json'
              ...
              , datareturn:function(results){
					if (results.json.type != 'video' && (results.json.url || results.json.thumbnail_url)) {
						return '<img src="' + (results.json.url || results.json.thumbnail_url) + '" class="'+settings.tagClass+'" />';
					}
    ...
    new $.fn.oembed.OEmbedProvider("dribbble", "photo", ["dribbble.com/shots/.+"], "http://api.dribbble.com/shots/$1?callback=?",
      ...
      templateData : function(data){if(!data.image_teaser_url)return false;
          return  '<img src="'+data.image_teaser_url+'" class="'+settings.tagClass+'"/>';
        }
      }),
    ...
    new $.fn.oembed.OEmbedProvider("visual.ly", "rich", ["visual\\.ly/.+"], null,
      ...
      , template : function(wm,email){
        return '<img src="http://gravatar.com/avatar/'+email.md5()+'.jpg" alt="on Gravtar" class="jqoaImg '+settings.tagClass+'">';
        }
      }),
    ...
    ,{templateRegex:/(http:\/\/(((?!users).)+)\.livejournal\.com|.*users\.livejournal\.com\/([^\/]+)).*/,
      templateData : function(data){if(!data.username)return false;
          return  '<div><img src="'+data.image+'" align="left" style="margin-right: 1em;" class="'+settings.tagClass+'" /><span class="oembedall-ljuser"><a href="http://'+data.username+'.livejournal.com/profile"><img src="http://www.livejournal.com/img/userinfo.gif" alt="[info]" width="17" height="17" /></a><a href="http://'+data.username+'.livejournal.com/">'+data.username+'</a></span><br />'+data.name+'</div>';
        }
      }),  
    ...
    ,{templateRegex:/.*facebook.com\/(people\/[^\/]+\/(\d+).*|([^\/]+$))/,
      templateData : function(data){ if(!data.id)return false;
          ...
          if(data.picture) out += '<a href="'+data.link+'"><img src="'+data.picture+'" class="'+settings.tagClass+'" /></a>';
          else out += '<img src="https://graph.facebook.com/'+data.id+'/picture" class="'+settings.tagClass+'"/>';
    ...
    {yql:{xpath:"//meta|//title|//link", from:'html'
        , datareturn:function(results){
            ...
            var code = $('<p/>');
            if(results['og:video'] && settings.includeRichMedia) {
              ...
              var img = $('<img src="'+results['og:image']+'" class="'+settings.tagClass+'">');
